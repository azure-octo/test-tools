...
class FileCache:
    ...
    def prefetch(self, prefetch_artifacts, batch_size=15):
        for i in range(batch_size - len(self.threads)):
            prefetch_artifact = next(prefetch_artifacts)
            if f"{prefetch_artifact.name}.{prefetch_artifact.id}" in self.seen_artifacts:
                continue
            self.seen_artifacts.add(f"{prefetch_artifact.name}.{prefetch_artifact.id}")
            new_thread = threading.Thread(target=self.get,args=(f"{prefetch_artifact.name}.{prefetch_artifact.id}", prefetch_artifact.archive_download_url))
...
class TestArtifacts:
    ...
    def get_next_artifact(self, test_suites=None):
        ...
        while True:
            ...
            for artifact in responses.pop(0).artifacts:
    ...
    def get_artifacts(self, batch_size=200):
        ...
        artifact_count = 0
        for artifact in self.get_next_artifact():
